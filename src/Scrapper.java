    ...
    protected static Response makeCurlRequest(String urlString, String method, String body, Map<String, String> headers, int minChars) throws Exception {
    	...
    	for (int i = 0; i < attempts; i++) {
    		try {
                ...
                if ("br".equals(response.headers().firstValue("Content-Encoding").orElse(null))) {
                    // Decompress Brotli-encoded content
                    BrotliInputStream brotliInputStream = new BrotliInputStream(new ByteArrayInputStream(response.body()));
                ...
                else if ("zstd".equals(response.headers().firstValue("Content-Encoding").orElse(null))) {
